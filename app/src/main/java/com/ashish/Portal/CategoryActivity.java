...
//////////home page fake list
        ...
        int listPosition=0;
        for(int x = 0; x< DBquerries.loadedCategoriesNames.size(); x++){
            if(DBquerries.loadedCategoriesNames.get(x).equals(title.toUpperCase())){
                listPosition=x;
            }
        }

        if(listPosition == 0){
            DBquerries.loadedCategoriesNames.add(title.toUpperCase());
            DBquerries.lists.add(new ArrayList<MyMallModel>());
            DBquerries.loadFragmentData(categoryRecyclerView,this, DBquerries.loadedCategoriesNames.size()-1,title);
        }else {
            myMallAdapter=new MyMallAdapter(DBquerries.lists.get(listPosition));
